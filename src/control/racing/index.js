...
export const run = () => {
  const { racing, game, games } = store.getState()
  const racingObj = new Racing(racing)
  racingObj.run()
  store.dispatch(addScore({ game, score: 10 }))
  const newScore = games[game].score + 10
  setSpeedAndLevels(newScore, newScore/10)
  store.dispatch(setRacing(racingObj.toJsObj()))
}
